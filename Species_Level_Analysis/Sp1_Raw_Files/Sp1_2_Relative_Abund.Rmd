---
title: "Plaque_Species_level_relative_abundance_data"
...
Joined_All_Species_otu = Joined_All_Species_otu %>% 
  ...
  as.matrix() %>% 
  prop.table(., 1) %>% # columnwise
  as.data.frame()
Joined_All_Species_otu
```
# check the colsum
```{r}
rowSums(Joined_All_Species_otu) %>%  head()
```
...
write.table(Plaque_Species_raw_OTU_table_TSS_with_meta_data,
            paste0(out_dir, "/Plaque_Species_relative_abundance_OTU_table_with_meta_data.txt"),
